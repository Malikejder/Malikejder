<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&height=260&color=0:000000,40:001a0f,100:00FF41&text=MALIK%20EJDER%20DURGUN&fontColor=00FF41&fontSize=42&fontAlignY=40&animation=twinkling&desc=%3E%20system.breach.exe%20initiated_&descAlignY=58&descSize=18&descColor=00FFC8"/>

<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=700&size=22&pause=1000&color=00FF41&background=000000&center=true&vCenter=true&width=900&height=45&lines=root%40soc%3A~%23+SOC+Analyst;root%40soc%3A~%23+Threat+Hunter;root%40soc%3A~%23+Blue+Team+Operator;root%40soc%3A~%23+Detection+Engineering;root%40soc%3A~%23+Incident+Response;root%40soc%3A~%23+Python+Security+Automation"/>

<br>

<a href="https://www.linkedin.com/in/malikejder-durgun-61571b39b"><img src="https://img.shields.io/badge/LINKEDIN-000000?style=for-the-badge&logo=linkedin&logoColor=00FF41&labelColor=000000"/></a>
<img src="https://komarev.com/ghpvc/?username=Malikejder&style=for-the-badge&color=00ff41&label=PROFILE+VIEWS&labelColor=000000"/>

<br><br>

<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=500&size=14&pause=1500&color=00FF41&background=000000&center=true&vCenter=true&width=700&height=25&lines=%3E+decrypting+identity...+100%25+complete;%3E+access+level%3A+authorized"/>

</div>

---

# 🖥️ TERMINAL

```console
root@soc:~# whoami

Name        : Malik Ejder Durgun
Role        : SOC Analyst
Speciality  : Blue Team Operations
Focus       : Threat Hunting | Detection Engineering
Automation  : Python
OS          : Linux
Status      : ONLINE
```

```console
root@soc:~# ls skills/

✔ Splunk Enterprise
✔ Wireshark
✔ Sigma Rules
✔ YARA
✔ MITRE ATT&CK
✔ Windows Event Logs
✔ Syslog Analysis
✔ IOC Investigation
✔ Python
✔ Bash
✔ Git
✔ Linux
```

---

# 🚀 FEATURED PROJECTS

| Project | Description |
|---------|-------------|
| 🧠 **Zerguz CTI** | Multi-source Cyber Threat Intelligence platform |
| 🍯 **Zerguz Honeypot** | Python honeypot for Blue Team monitoring |
| 🔐 **Zerguz Encryption** | AES-256-GCM encryption utility |

---

# 🏆 VERIFIED CERTIFICATIONS

- Cisco Introduction to Cybersecurity
- Cisco Cyber Threat Management
- Cisco Cybersecurity Defense Analyst Career Path
- Cisco Ethical Hacker
- TryHackMe SOC Level 1
- BTK Academy Cybersecurity Trainings

---

# ⚙️ TECHNOLOGY

<p align="center">
<img src="https://skillicons.dev/icons?i=python,bash,linux,git,vscode"/>
</p>

<p align="center">
<img src="https://img.shields.io/badge/Splunk-black?style=for-the-badge&logo=splunk"/>
<img src="https://img.shields.io/badge/Wireshark-1679A7?style=for-the-badge&logo=wireshark&logoColor=white"/>
<img src="https://img.shields.io/badge/Cisco-1BA0D7?style=for-the-badge&logo=cisco&logoColor=white"/>
<img src="https://img.shields.io/badge/MITRE_ATT%26CK-FF6A00?style=for-the-badge"/>
<img src="https://img.shields.io/badge/YARA-111111?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Sigma-1F2937?style=for-the-badge"/>
</p>

---

# 📊 GITHUB DASHBOARD

<p align="center">
<img width="60%" src="https://streak-stats.demolab.com/?user=Malikejder&hide_border=true&background=000000&ring=00FF41&fire=00FFC8&currStreakLabel=00FF41&sideLabels=00FF90&currStreakNum=00FF41&sideNums=00FF90&dates=006644&border=00FF41"/>
</p>

<p align="center">
<img width="98%" src="https://github-readme-activity-graph.vercel.app/graph?username=Malikejder&hide_border=true&bg_color=000000&color=00FF41&line=00FFC8&point=00FF90&area=true&area_color=00FF41"/>
</p>


> ⚠️ Bu kartlar canlı GitHub API'sinden veri çekiyor. Üçüncü parti (vercel/demolab) ücretsiz sunucuları yoğun saatlerde 429/500 hatası verebilir — geçici bir bozulmaysa sayfayı birkaç dakika sonra yenilemen yeterli. Kalıcı çözüm için kendi Vercel hesabında `github-readme-stats`'ı fork'layıp deploy edebilirsin (istersen adım adım anlatırım).

<details>
<summary>🖴 Statik Yedek Panel (her zaman çalışır, veri sabit)</summary>

```console
root@soc:~# cat github_stats.log

┌─────────────────────────────────────────────┐
│  GITHUB ACTIVITY :: OFFLINE-SAFE SNAPSHOT    │
├─────────────────────────────────────────────┤
│  Primary Stack ..... Python / Bash           │
│  Repos Tracked ..... Zerguz CTI Suite        │
│  Focus Area ........ Blue Team Tooling       │
│  Commit Style ...... Automation-first        │
│  Status ............ [■■■■■■■■□□] ACTIVE     │
└─────────────────────────────────────────────┘

[OK] snapshot loaded — no network dependency
```

</details>

---

# 📡 CONTACT

- LinkedIn: https://www.linkedin.com/in/malikejder-durgun-61571b39b
- Email: durgunmalikejder725@gmail.com

---

```text
╔══════════════════════════════════════════════╗
║            SECURITY STATUS PANEL            ║
╠══════════════════════════════════════════════╣
║ Threat Monitoring ...... ACTIVE             ║
║ Detection Rules ....... LOADED              ║
║ IOC Intelligence ...... UPDATED             ║
║ Incident Response ..... READY               ║
║ SIEM Connection ....... CONNECTED           ║
║ Analyst Clearance ..... LEVEL 1             ║
╚══════════════════════════════════════════════╝

Access Granted.
```

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&section=footer&height=120&color=0:00FF41,100:000000"/>

<br>

<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=16&pause=2000&color=00FF41&background=000000&center=true&vCenter=true&width=600&height=30&lines=%3E+connection+terminated_"/>

</div>
